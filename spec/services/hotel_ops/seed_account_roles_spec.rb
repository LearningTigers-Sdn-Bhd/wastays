...
RSpec.describe HotelOps::SeedAccountRoles do
  ...
  before do
    %w[manage_account manage_hotel_profile manage_room_types manage_rates manage_inventory view_bookings view_financial_status manage_bookings void_bookings view_guest_phone manage_guest_arrival view_audit_logs export_audit_logs manage_users manage_room_status post_charges post_folio_charges post_folio_payments execute_folio_refunds post_folio_adjustments post_folio_corrections post_folio_write_offs manage_folio_windows manage_folio_movements view_reports view_payouts manage_requests manage_night_audit].each do |slug|
