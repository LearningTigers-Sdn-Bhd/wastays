...
RSpec.describe NightAudits::DetectDueOuts do
  ...
  it "moves checked-in bookings due on or before the business date to due_out_detected" do
    ...
    expect(result.detected.pluck("booking_id")).to contain_exactly(due_today.id, overdue.id)
    expect(due_today.reload.status).to eq("due_out_detected")
    expect(overdue.reload.status).to eq("due_out_detected")
  end

  it "leaves future, non-checked-in, and already detected bookings unchanged" do
    ...
    detected = create(:booking, hotel: hotel, status: "due_out_detected", check_out: business_date, checked_in_at: 1.day.ago)
    ...
    expect(detected.reload.status).to eq("due_out_detected")
  end
  ...
  it "records an immutable booking audit log linked to the night audit and business date" do
    ...
    expect(log.new_value).to eq("status" => "due_out_detected")
