...
RSpec.describe NightAudits::OperationalChangeGuard do
  ...
  it "allows the matching active night audit to perform controlled changes" do
    ...
    expect(described_class.call!(hotel: hotel, action: :detect_due_out, night_audit: audit)).to be(true)
  end
  ...
  it "rejects a completed, cross-hotel, or wrong-date audit bypass" do
    ...
    expect do
      described_class.call!(hotel: hotel, action: :detect_due_out, night_audit: completed)
    end.to raise_error(described_class::OperationalChangeBlocked)

    expect do
      described_class.call!(hotel: hotel, action: :detect_due_out, night_audit: other_audit)
    end.to raise_error(described_class::OperationalChangeBlocked)
  end
end
