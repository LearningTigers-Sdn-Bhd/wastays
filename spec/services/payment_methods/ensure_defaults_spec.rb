...
RSpec.describe PaymentMethods::EnsureDefaults do
  ...
  it "wraps the seven existing payment codes idempotently" do
    described_class.call(hotel)

    expect(hotel.hotel_payment_methods.count).to eq(7)
    expect(hotel.hotel_payment_methods.joins(:transaction_code).pluck("transaction_codes.system_key"))
      .to match_array(%w[cash_payment cash_prepayment card_payment card_prepayment bank_payment gateway_manual_recovery_payment ota_collected_payment])
    expect(hotel.hotel_payment_methods.find_by!(default_cash: true).transaction_code.system_key).to eq("cash_payment")
    expect(hotel.hotel_payment_methods.joins(:transaction_code).where(transaction_codes: { system_key: %w[cash_prepayment card_prepayment bank_payment ota_collected_payment] }))
