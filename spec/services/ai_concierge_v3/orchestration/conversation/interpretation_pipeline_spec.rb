...
RSpec.describe AiConciergeV3::Orchestration::Conversation::InterpretationPipeline do
  ...
  it "builds summary, calls the interpreter, and validates the interpretation" do
    allow_any_instance_of(AiConciergeV3::Agents::InterpreterAgent).to receive(:call).and_return(interpretation)
  ...
  it "raises when interpretation schema is invalid" do
    allow_any_instance_of(AiConciergeV3::Agents::InterpreterAgent).to receive(:call).and_return({ "intent" => "greeting" })
  ...
  it "resets stale booking context for a fresh booking request without details" do
    ...
    payload = AiConciergeV3::State::ConversationTaskManager.new(slots_payload: {}).activate_booking(branch, pending_question: "booking_timing")
