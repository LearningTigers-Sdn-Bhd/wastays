...
RSpec.describe HotelPortal::Reports::PayoutsExportService do
  ...
  it "generates upcoming csv/xls/pdf" do
    booking = instance_double(Booking, confirmation_token: "WS-ABC", checked_out_at: Time.zone.local(2026, 5, 7, 10, 0), net_amount: 120.to_d)
    ...
    expect(service.generate_csv).to include("Booking Ref,Checked Out At,Net Amount")
  ...
  it "generates paid csv" do
    batch = instance_double(PayoutBatch, period_start: Date.new(2026, 5, 1), period_end: Date.new(2026, 5, 7), payout_at: Date.new(2026, 5, 8), payout_reference: "PO-1", amount: 550.to_d)
    ...
    expect(service.generate_csv).to include("Period,Settled At,Reference,Net Amount")
  end
end
