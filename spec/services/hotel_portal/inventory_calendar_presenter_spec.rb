...
RSpec.describe HotelPortal::InventoryCalendarPresenter do
  ...
  describe '#format_price' do
    it 'formats price with currency symbol and handles decimals' do
      expect(presenter.send(:format_price, 200)).to eq("RM200")
      expect(presenter.send(:format_price, 200.5)).to eq("RM200.5")
      expect(presenter.send(:format_price, 200.55)).to eq("RM200.55")
    end

    it 'uses $ for USD' do
      p = described_class.new(hotel: hotel, start_date: start_date, end_date: end_date, display_currency: "USD")
      expect(p.send(:format_price, 100)).to eq("$100")
    end
  end
end
