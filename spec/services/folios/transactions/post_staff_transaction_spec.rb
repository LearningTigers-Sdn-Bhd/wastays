...
RSpec.describe Folios::Transactions::PostStaffTransaction, frozen_time: Time.zone.local(2026, 6, 10, 3) do
  ...
  it "rolls back the parent transaction if generated tax posting fails" do
    ...
    allow(Folios::Transactions::InsertTransaction).to receive(:new).and_wrap_original do |method, **kwargs|
      if kwargs[:category] == "tax"
        instance_double(Folios::Transactions::InsertTransaction, call: Folios::Transactions::TransactionResult.failure("tax failed"))
      else
        method.call(**kwargs)
      end
    end
