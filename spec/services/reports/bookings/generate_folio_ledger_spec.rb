...
RSpec.describe Reports::Bookings::GenerateFolioLedger do
  ...
  it "generates current folio ledger rows as PDF" do
    ...
    # The reference is the title; the eyebrow above it says what the reference is.
    expect(text).to include("FOLIO LEDGER")
    expect(text).to include("LED-26300123/1")
    # Three party blocks rather than a metadata strip, so the guest address has room.
    expect(text).to include("GUEST DETAILS", "Guest", "Ledger & Guest", "Address")
    expect(text).to include("CONTACT DETAILS", "Booking no.", "Confirmation", "BK-LEDGER")
    expect(text).to include("ACCOUNT DETAILS", "Account ref", "LED-26300123")
    expect(text).to include("Status", "Open", "Currency", "MYR")
    expect(text).to include("Ledger transactions")
    ...
    # Printed by and generated moved out of the old footer into the account grid; the
    # shared footer carries confidentiality, attribution and pagination instead.
    expect(text).to include("Printed by", "Platform Admin")
    expect(text).to include("Generated", "22 Jun 2026")
    expect(text).to include("Confidential", "Generated by")
