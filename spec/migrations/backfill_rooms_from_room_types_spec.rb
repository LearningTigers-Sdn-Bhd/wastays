...
RSpec.describe BackfillRoomsFromRoomTypes do
  ...
  it "backfills trimmed room numbers with positions and inherited groups" do
    hotel = create(:hotel)
  ...
  it "reports hotel-wide collisions before it inserts rooms" do
    ...
    expect { ActiveRecord::Migration.suppress_messages { migration.up } }
      .to raise_error(ActiveRecord::MigrationError, /hotel #{hotel.id}.*room "101".*#{first.id}.*#{second.id}.*Rename each duplicate/m)
