...
RSpec.describe 'Admin::Hotels', type: :request do
  ...
  describe 'GET /admin/hotels/new' do
    it 'shows the redesigned hotel creation workspace' do
      ...
      expect(response.body).to include('class="mt-2 text-sm font-medium text-slate-600 sm:text-base">Set up the hotel profile before the property starts receiving bookings.')
      expect(response.body).to include('Set up the hotel profile before the property starts receiving bookings.')
