...
RSpec.describe "Hotel portal OTA settlement report", type: :request do
  ...
  it "renders a currency-safe provider reconciliation and report navigation entry" do
    ...
    expect(page).to have_no_button("Apply filters")
    expect(page).to have_link("Clear")
    filters = page.find("form[data-controller='auto-submit']")
    expect(filters["data-action"]).to include("input->auto-submit#submit", "change->auto-submit#submitNow")
