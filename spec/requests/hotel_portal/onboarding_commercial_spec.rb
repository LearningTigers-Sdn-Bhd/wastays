...
RSpec.describe "Hotel onboarding commercial phase", type: :request do
  ...
  describe "payment methods" do
    ...
    it "shows the standard payment codes rather than offering them for editing" do
      ...
      expect(response.body).to include("Cash Payment", "Cash Prepayment", "Card Payment", "Card Prepayment")
      ...
      card_prepayment_row = response.parsed_body.css("tr").find { |row| row.text.include?("Card Prepayment") }
      expect(card_prepayment_row).to be_present
      expect(card_prepayment_row.at_css("input[name$='[guest_advance]']")&.[]("value")).to eq("true")
      expect(card_prepayment_row.at_css("input[name$='[default_cash]']")&.[]("value")).to eq("false")
    end
