...
RSpec.describe PanelsUI::Tooltip, type: :component do
  ...
  it "wires the controller, values, and hover/focus action contract" do
    ...
    expect(page).to have_css("span.tooltip-root[data-controller='ui--tooltip']")
    root = page.find("span.tooltip-root")
    expect(root["data-ui--tooltip-placement-value"]).to eq("top")
    expect(root["data-ui--tooltip-offset-value"]).to eq("6.0")
    expect(root["data-ui--tooltip-delay-value"]).to eq("120")
    expect(root["data-ui--tooltip-tooltip-id-value"]).to eq("tip")
    expect(root["data-action"]).to include("mouseenter->ui--tooltip#show")
    expect(root["data-action"]).to include("focusin->ui--tooltip#show")
    expect(root["data-action"]).to include("keydown->ui--tooltip#onKeydown")
  end
  ...
  it "renders the trigger content alongside a closed popover bubble" do
    ...
    expect(page).to have_css(
      "#tip[role='tooltip'][popover='manual'][data-state='closed'][data-ui--tooltip-target='bubble']",
  ...
  it "renders an arrow target by default" do
    ...
    expect(page).to have_css(
      "#tip .floating-arrow[aria-hidden='true'][data-ui--tooltip-target='arrow']",
      visible: :all
    )
  end
  ...
  it "normalizes placement for Floating UI and merges bubble classes" do
    ...
    expect(page).to have_css("span.tooltip-root[data-ui--tooltip-placement-value='bottom-end']")
  ...
  it "falls back to a top placement for an unknown placement" do
    ...
    expect(page).to have_css("span.tooltip-root[data-ui--tooltip-placement-value='top']")
  end
