...
RSpec.describe PanelsUI::DateTimePicker, type: :component do
  ...
  it "renders a hidden field with a calendar + segmented time popover" do
    ...
    expect(root["data-controller"]).to include("ui--date-time-picker")
    expect(root["data-size"]).to eq("md")

    expect(page).to have_css("input#booking_arrives_at[type='hidden'][data-ui--date-time-picker-target='input']", visible: :all)
    expect(page).to have_css("button.panel-input.panel-date-time-picker__display[data-ui--date-time-picker-target='trigger']")
    expect(page).to have_css("calendar-date[data-ui--date-time-picker-target='calendar'] calendar-month")
    expect(page).to have_css("button.panel-date-time-picker__time-trigger", text: "Time")
    expect(page).to have_css(".panel-time-control[data-ui--date-time-picker-target='timeControl']")
    expect(page).to have_css(".panel-date-time-picker__time-popover[data-ui--popover-target='panel']")
  end

  it "wires the popover outlet to the nested popover root" do
    render_inline(described_class.new(form: form_for, attribute: :arrives_at))

    selector = page.find(".panel-date-time-picker")["data-ui--date-time-picker-ui--popover-outlet"]
  ...
  it "passes date-only bounds to the calendar and keeps step as a data-value" do
    ...
    root = page.find(".panel-date-time-picker")
    expect(root["data-ui--date-time-picker-min-value"]).to eq("2026-07-12T09:00")
    expect(root["data-ui--date-time-picker-max-value"]).to eq("2026-12-31T17:00")
    expect(root["data-ui--date-time-picker-step-value"]).to eq("15")
  ...
  it "renders a two-month datetime range with separate start and end time segments" do
    ...
    root = page.find(".panel-date-time-picker")
    expect(root["data-ui--date-time-picker-mode-value"]).to eq("range")
    expect(page).to have_css("calendar-range[months='2'] calendar-month[offset='1']")
    expect(page).to have_css("[data-ui--date-time-picker-target='startTimeControl']")
    expect(page).to have_css("[data-ui--date-time-picker-target='endTimeControl']")
  ...
  it "renders 12-hour seconds controls and preserves second-precision values" do
    ...
    root = page.find(".panel-date-time-picker")
    expect(root["data-ui--date-time-picker-hour-cycle-value"]).to eq("12")
    expect(root["data-ui--date-time-picker-precision-value"]).to eq("seconds")
