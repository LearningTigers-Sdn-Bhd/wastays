...
RSpec.describe GuestUI::ActionCard, type: :component do
  it "makes the whole card a labelled link with a visible focus treatment" do
    render_inline(described_class.new(label: "Report an issue", hint: "Tell the hotel team what is wrong.",
                                      icon: "circle-alert", tone: :issue, href: "/requests/new"))

    expect(page).to have_css("a.guest-action-card[href='/requests/new']", text: "Report an issue")
