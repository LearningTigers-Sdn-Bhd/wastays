...
RSpec.describe GuestUI::Chat::Status, type: :component do
  it "says who is answering, under a stable id a live update can find" do
    ...
    expect(page).to have_css("p#concierge-chat-status.guest-chat__status", text: "Our front desk replies here")
    expect(page).to have_css(".guest-chat__status[data-tone='muted']")
  end
  ...
  it "marks a person holding the thread as worth noticing" do
    ...
    expect(page).to have_css(".guest-chat__status[data-tone='accent']")
  end
