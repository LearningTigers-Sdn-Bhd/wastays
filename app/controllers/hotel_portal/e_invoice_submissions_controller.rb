...
module HotelPortal
  class EInvoiceSubmissionsController < HotelPortal::BaseController
    ...
    # Submitting or cancelling sends documents to LHDN under the hotel's tax
    # identity, so writes are held to the same bar as other booking changes.
    def authorize_view_e_invoices!
      raise Pundit::NotAuthorizedError unless current_user.has_permission?("view_bookings", hotel: current_hotel)
    end

    def authorize_manage_e_invoices!
