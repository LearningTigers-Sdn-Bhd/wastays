...
class HotelCounter < ApplicationRecord
  ...
  validates :sequence_year, presence: true, numericality: { only_integer: true, greater_than_or_equal_to: 2000 }
  before_validation :assign_sequence_year
  ...
  def self.increment!(hotel:, type:, year:, floor: nil)
    counter = find_or_create_by!(hotel: hotel, counter_type: type, sequence_year: year)
  ...
  def assign_sequence_year
    self.sequence_year ||= DocumentIdentifiers::Issuer.sequence_year(hotel:) if hotel
  end
end
