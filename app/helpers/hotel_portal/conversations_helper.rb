...
module HotelPortal
  module ConversationsHelper
    ...
    def conversation_inbox_tabs(counts)
      [
        { name: "all", label: "Open", icon: "inbox", count: counts[:open], params: {} },
        { name: "unread", label: "Unread", icon: "mail", count: counts[:unread], params: { filter: "unread" } },
        { name: "awaiting_staff", label: "Waiting", icon: "user-round", count: counts[:awaiting_staff], params: { filter: "awaiting_staff" } },
        { name: "closed", label: "Closed", icon: "archive", count: nil, params: { status: "closed" } }
      ]
    end
