...
module CorporatePortal
  module AccountsReceivable
    class IndexPresenter
      ...
      def filters_active?
        query.present? || selected_status.present? || selected_hotel_id.present? || due_on.present? || selected_balance.present?
      end
      ...
      def summary_metrics
        [
          ...
          Metric.new(
            ...
            amounts: formatted_amounts(received_this_month_amounts),
      ...
      def base_scope
        ArInvoice
          ...
          .includes(:hotel, booking_folio: :booking, hotel_corporate_account: :corporate_account)
      ...
      def search_scope(scope)
        ...
        scope
          .left_joins(:hotel, booking_folio: :booking)
          .where(
            <<~SQL.squish,
              CAST(ar_invoices.invoice_number AS TEXT) ILIKE :query
              OR hotels.name ILIKE :query
      ...
      def corporate_invoices_scope
        ArInvoice
          .joins(:hotel_corporate_account)
          .where(hotel_corporate_accounts: { corporate_account_id: account.id })
      end

      def corporate_payments_scope
        ArPayment
          .joins(:hotel_corporate_account)
          .where(hotel_corporate_accounts: { corporate_account_id: account.id })
      end
      ...
      def open_ar_amounts
        corporate_invoices_scope.with_open_balance.group(:currency).sum(:outstanding_amount)
      end

      def overdue_amounts
        corporate_invoices_scope.with_open_balance
          .where(ArInvoice.arel_table[:due_on].lt(Date.current))
          .group(:currency).sum(:outstanding_amount)
      end

      def due_soon_amounts
        corporate_invoices_scope.with_open_balance
          .where(due_on: Date.current..(Date.current + 7.days))
          .group(:currency).sum(:outstanding_amount)
      end

      def received_this_month_amounts
        corporate_payments_scope
      ...
      def formatted_amounts(amounts)
        ...
        normalized = { (account.hotel_corporate_accounts.first&.credit_currency || "MYR") => 0.to_d } if normalized.empty?
      ...
      class Row
        ...
        delegate :booking_folio, to: :invoice
