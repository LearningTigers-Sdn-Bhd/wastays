...
module HotelPortal
  class RatePlanWizard
    # One wizard step: what this plan charges for a single room category.
    #
    # The step is scoped to one category on purpose. Capacity varies between
    ...
    # does the second. With one category per step, max_adults is known and the
    ...
    class RoomPricing
      ...
      # The full matrix this step contributes. Empty for a per-room plan, which
      ...
      def anchor_is_available
        ...
        errors.add(:base, "#{room_type&.name} has no standard rate to derive from. Set one on the room category, or price this step manually.")
      end
