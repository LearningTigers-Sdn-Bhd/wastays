    ...
    class RouteCodeToBillingParty
      ...
      def call
        ...
        FolioRoutingRule.transaction do
          ...
          Folios::Forecasts::SyncForecastedCharges.call(booking_folio: @booking.booking_folio || @target_folio)
          success(rule)
        end
      ...
      def success(rule) = Folios::Routing::RuleResult.success(rule: rule)
      def failure(error) = Folios::Routing::RuleResult.failure(error)
    end
  end
end
