  ...
  module Routing
    class ApplyExistingCharges
      ...
      def call
        ...
        moved = []
        preview = Folios::Routing::PreviewExistingCharges.call(rule: @rule)
      ...
      def move_transaction(transaction)
        ...
        if parent_id
          ...
          tax_routes = Folios::Transactions::AttachedTaxTransactions.call(parent).each_with_object({}) do |tax, routes|
          ...
          return Folios::Transactions::MoveTransaction.call(transaction: parent, target_folio: parent.booking_folio,
            user: @actor, reason: @reason, tax_routes: tax_routes)
        end

        tax_routes = Folios::Transactions::AttachedTaxTransactions.call(transaction).each_with_object({}) do |tax, routes|
          child_rule = @rule.booking.folio_routing_rules.active.find_by(transaction_code_id: tax.transaction_code_id)
          routes[tax.id] = child_rule.target_folio_id if child_rule
        end
        Folios::Transactions::MoveTransaction.call(transaction:, target_folio: @rule.target_folio, user: @actor,
          reason: @reason, tax_routes: tax_routes)
      end

      def success(transactions)
        Folios::Routing::BatchResult.success(transactions: transactions)
      end

      def failure(message, transactions = [])
        Folios::Routing::BatchResult.failure(message, transactions: transactions)
      end
    end
  end
end
