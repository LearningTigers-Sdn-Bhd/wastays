...
module Folios
  module Maintenance
    class RecoverMissingFolio
      ...
      def create_folio!
        Folios::Lifecycle::BuildPrimaryFolio.call(booking: @booking, actor: @actor, hotel: @hotel)
      end
      ...
      def success(folio:, created:)
        Folios::Maintenance::RecoveryResult.success(folio: folio, "created?": created, message: created ? "Folio recovered." : "Folio already exists.")
      end

      def failure(error)
        Folios::Maintenance::RecoveryResult.failure(error, "created?": false, message: error)
      end
    end
  end
end
