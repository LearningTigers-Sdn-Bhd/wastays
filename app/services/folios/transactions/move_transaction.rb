  ...
  module Transactions
    class MoveTransaction
      ...
      def success(transactions)
        Folios::Transactions::MoveResult.success(transactions: transactions, transaction: transactions.first, operation_key: @operation_key)
      end

      def failure(error)
        Folios::Transactions::MoveResult.failure(error, transactions: [])
      end
    end
  end
end
