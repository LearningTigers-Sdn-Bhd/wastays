...
module Reports
  module Bookings
    class GenerateFolioRecords
      ...
      attr_reader :booking, :hotel, :folio
      ...
      def hotel_info_rows
        [
          [ "Hotel Name", hotel.name.presence ],
      ...
      def guest_folio_detail_rows
        [
          [ "Guest Name", guest_value(booking.guest_name) ],
          [ "Nationality", guest_value(booking.guest_country) ],
      ...
      def validate_closed_folio!
        return if folio&.closed?

        raise UnavailableError, "Invoice is only available for bookings with a closed folio."
      end
      ...
      def active_transactions
        @active_transactions ||= folio.folio_transactions
                                       .includes(:transaction_code, :user)
                                       .order(:posting_date, :created_at, :id)
                                       .reject { |transaction| hidden_reversal_noise?(transaction) }
      end
      ...
      def source_transaction_code_for(transaction)
        source_id = tax_line(transaction)["source_transaction_code_id"].presence ||
          ...
          parent_transaction_for(transaction)&.transaction_code_id
      ...
      def hotel_address
        [ hotel.address, hotel.city, hotel.country ].filter_map(&:presence).uniq.join(", ").presence
      end

      def hotel_contact
        [ hotel.contact_phone, hotel.contact_email ].filter_map(&:presence).join(" · ").presence
      end
