    ...
    class GenerateCombinedInvoices
      ...
      def summary_row(invoice)
        ...
        [
          ...
          @recipient.name,
          records.currency,
          HotelPortal::Reports::Exports::PdfTheme.money(records.total_due)
        ]
      end
      ...
      def draw_currency_totals(pdf)
        ...
        rows = totals.sort.map do |currency, amount|
          [ { content: "Total (#{currency})", font_style: :bold }, { content: HotelPortal::Reports::Exports::PdfTheme.money(amount), align: :right, font_style: :bold } ]
        end
