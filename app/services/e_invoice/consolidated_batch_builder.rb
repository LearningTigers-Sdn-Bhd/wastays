...
module EInvoice
  class ConsolidatedBatchBuilder
    ...
    def supplier
      @supplier ||= @context&.intermediary? ? hotel_supplier_profile : wastays_supplier_profile
    end
    ...
    def wastays_supplier_profile
      {
        tin: @creds[:tin].to_s.presence || raise(ArgumentError, "myinvois.tin not configured"),
        brn: @creds[:brn].to_s.presence || raise(ArgumentError, "myinvois.brn not configured"),
        name: @creds[:name].to_s.presence || "Jesselton Pixel Sdn Bhd",
        msic_code: @creds[:msic_code].to_s.presence || "63120",
        ...
        address_line1: @creds[:address].to_s.presence || NA_VALUE,
        ...
        country_code: @creds[:country_code].to_s.presence || ORIGIN_COUNTRY_CODE
      }
    end
