...
class FolioInvoicePdfService
  ...
  def initialize(booking, printed_by: nil)
    @booking = booking
    @presenter = Invoices::GuestFolioPresenter.new(booking: booking, printed_by: printed_by)
  end
  ...
  def draw_header(pdf)
    ...
    pdf.move_up 32
    pdf.fill_color DARK_GREEN
    pdf.text @presenter.document_title, size: 18, style: :bold, align: :right
    pdf.move_down 12
  ...
  def draw_hotel_information(pdf)
    rows = @presenter.hotel_info_rows
  ...
  def draw_guest_booking_details(pdf)
    left_rows = @presenter.guest_folio_detail_rows
    right_rows = @presenter.booking_stay_detail_rows
  ...
  def draw_transactions(pdf)
    rows = [
      [
        ...
        header_cell("Net (#{@presenter.currency})", align: :right),
        header_cell("Charges (#{@presenter.currency})", align: :right),
        header_cell("Gross (#{@presenter.currency})", align: :right)
      ]
    ]

    @presenter.transaction_rows.each do |row|
      rows << [
  ...
  def money_cell_text(amount, credit:)
    ...
    return @presenter.credit_amount(amount) if credit

    @presenter.amount(amount)
  end

  def draw_summary(pdf)
    section_title(pdf, "SUMMARY (#{@presenter.currency})", width: summary_width(pdf), position: :right)

    rows = @presenter.summary_rows.map do |row|
      amount = row.credit ? @presenter.credit_amount(row.amount) : @presenter.amount(row.amount)
      [
  ...
  def draw_legend(pdf)
    legend_rows = @presenter.legend_rows
  ...
  def draw_footer(pdf)
    pdf.repeat(:all) do
      pdf.bounding_box([ pdf.bounds.left, 20 ], width: pdf.bounds.width, height: 16) do
        ...
        pdf.text "Printed by: #{@presenter.printed_by}", size: 7, align: :left
      end
    end

    pdf.number_pages "Page <page> of <total>",
      at: [ pdf.bounds.right - 75, 20 ],
      size: 7,
      color: TEXT_MUTED
  end
