module AiConciergeV3
  module Orchestration
    ...
    def call
      ...
      interpretation["conversation_signals"]["end_conversation"] = true if end_conversation_message?

      if interpretation.dig("conversation_signals", "starts_new_booking_branch")
        new_payload = State::BranchManager.new(slots_payload: conversation_state.slots_payload).archive_completed_active
    ...
    def process_decision(prospect:, conversation_state:, interpretation:, active_branch:, decision:)
      ...
      when :ask_booking_timing
        payload = State::BranchManager.new(slots_payload: conversation_state.slots_payload).start_branch(active_branch)
      ...
      when :ask_specific_timing
        payload = State::BranchManager.new(slots_payload: conversation_state.slots_payload).start_branch(active_branch)
        build_and_persist_response(prospect:, conversation_state:, interpretation:, slots_payload: payload, reply_type: :ask_specific_timing, active_topic: "booking_search", active_flow: "booking_search", pending_question: "specific_timing", action_name: "request_quote", extra_context: { month_label: month_label(active_branch) })
      when :ask_duration
        payload = State::BranchManager.new(slots_payload: conversation_state.slots_payload).start_branch(active_branch)
      ...
      when :ask_guest_count
        payload = State::BranchManager.new(slots_payload: conversation_state.slots_payload).start_branch(active_branch)
        build_and_persist_response(prospect:, conversation_state:, interpretation:, slots_payload: payload, reply_type: :ask_guest_count, active_topic: "booking_search", active_flow: "booking_search", pending_question: "guest_count", action_name: "request_quote", extra_context: { month_label: month_label(active_branch), check_in: active_branch["check_in"] })
    ...
    def handle_resume(prospect:, conversation_state:, interpretation:)
      slots_payload, resumed = State::BranchManager.new(slots_payload: conversation_state.slots_payload).resume_latest
      ...
      if active_branch["suggested_options"].present?
        search_params = active_branch.slice("check_in", "check_out", "adults", "children", "room_count")
        if search_params["check_in"].blank?
          first_opt = active_branch["suggested_options"].first["options"].first
          search_params["check_in"] = first_opt["check_in"]
          search_params["check_out"] = first_opt["check_out"]
        end

        reply = Agents::MessengerAgent.new(hotel: hotel, context: {
        ...
        return ResponsePayloadBuilder.new(reply_message: reply, needs_human_support: false, action_name: "request_quote", prospect_public_id: prospect.public_id).call
      end
    ...
    def handle_end_conversation(prospect:, conversation_state:, interpretation:)
      slots_payload = State::BranchManager.new(slots_payload: conversation_state.slots_payload).archive_completed_active
      persist_state(
        conversation_state,
        slots_payload: slots_payload,
        interpretation: interpretation,
    ...
    def handle_search_options(prospect:, conversation_state:, interpretation:, active_branch:)
      ...
      if options.empty?
        ...
        search_params = active_branch.slice("check_in", "check_out", "adults", "children", "room_count")
        if search_params["check_in"].blank? && options.present?
    ...
    def handle_confirmation_yes(prospect:, conversation_state:, interpretation:, active_branch:)
      ...
      active_branch["confirmation_candidate"] = nil
      payload = State::BranchManager.new(slots_payload: conversation_state.slots_payload).start_branch(active_branch)
      payload = State::BranchManager.new(slots_payload: payload).archive_completed_active

      build_and_persist_response(prospect:, conversation_state:, interpretation:, slots_payload: payload, reply_type: :booking_link_ready, active_topic: nil, active_flow: nil, pending_question: nil, action_name: nil, extra_context: { result: result.merge("selected_option" => selected_option) }, flow_status: "ended", end_reason: "booking_url_generated")
    end
    ...
    def info_slots_payload(conversation_state:, active_branch:, pause:)
      ...
      slots_payload = State::BranchManager.new(slots_payload: slots_payload).start_branch(active_branch)
      return slots_payload unless pause

      State::BranchManager.new(slots_payload: slots_payload).pause_active(topic: "booking_search", pending_question: conversation_state.pending_question || "select_option")
    end
    ...
    def filtered_slots_for_merge(slots:, pending_question:, conversation_signals:)
      ...
      timing_keys.each { |key| filtered_slots.delete(key) } unless explicit_timing_in_message?(pending_question)
      ...
      apply_guest_count_guards!(filtered_slots, pending_question)
      ...
      when "specific_timing"
        # Keep timing slots if we are clarifying specific timing
      end

      # Deterministic guard against LLM hallucinating check_in for vague month requests
      if filtered_slots["check_in"].present? && !message_contains_specific_date?
        begin
          parsed_date = Date.parse(filtered_slots["check_in"])
          filtered_slots["target_month"] = parsed_date.month if filtered_slots["target_month"].blank?
          filtered_slots["target_year"] = parsed_date.year if filtered_slots["target_year"].blank?
        rescue Date::Error
          # Ignore parse errors
        end
        filtered_slots.delete("check_in")
        filtered_slots.delete("check_out")
      end

      # Deterministic guard against LLM hallucinating month_segment
      if filtered_slots["month_segment"].present? && !message_contains_month_segment?
        filtered_slots.delete("month_segment")
      end
    ...
    def explicit_timing_in_message?(pending_question = nil)
      normalized = message.downcase
    ...
    def apply_guest_count_guards!(filtered_slots, pending_question)
      if explicit_people_total_in_message?(pending_question)
        filtered_slots["party_size_total"] = extracted_people_total(pending_question) if extracted_people_total(pending_question).positive?
        ...
        filtered_slots.delete("party_size_total")
      end

      if explicit_adults_in_message? && !explicit_children_in_message?
        filtered_slots["adults"] = extracted_adult_count if extracted_adult_count.positive?
        filtered_slots.delete("children") unless explicit_people_total_in_message?(pending_question)
      end

      if explicit_children_in_message? && !explicit_adults_in_message?
        filtered_slots["children"] = extracted_children_count if extracted_children_count.positive?
      end
    end

    def explicit_people_total_in_message?(pending_question)
    ...
    def extracted_people_total(pending_question)
    ...
    def resolve_selection_follow_up(prospect:, conversation_state:, interpretation:, active_branch:, pending_question:)
      ...
      when "room_type_requires_option_number"
        ...
        payload = State::BranchManager.new(slots_payload: conversation_state.slots_payload).start_branch(branch)
    ...
    def end_conversation_message?
      ...
      normalized.match?(/\A(?:stop|bye|goodbye|thanks|thank you|that's all|thats all|end chat|nevermind|never mind|forget|forget it|no thanks|not now)\z/)
    end
