...
Folio Forecasted Charges implemented on June 10, 2026: nightly room and tax charges are now pre-computed at check-in and displayed as pending forecast rows on the folio page. Night audit actualizes forecasts as charges are posted, including idempotent retry paths where the charge already exists. Booking date/room/rate changes, catch-up posting, and early departure now reconcile through `Folios::SyncForecastedCharges` so already-posted nights are not recreated as pending forecasts. Checkout syncs forecasts before validation and uses active forecast rows to detect missing charges.

Folio Forecasted Charges hardening completed on June 11, 2026: forecast line construction now flows through `Folios::ForecastedChargeLines`, posting key formats through `Folios::ChargePostingKeys`, and forecast generation is retry-safe. Reversing an actualized nightly charge now supersedes the linked forecast and runs forecast sync, so checkout validation can catch reversed/missing nightly charges.
