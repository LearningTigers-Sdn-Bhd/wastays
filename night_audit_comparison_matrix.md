...
| Folio Posting | Strong but incomplete | Checked-in-only room/tax posting, immutable transactions, locks, posting-date controls, and database duplicate protection exist. Run-level skipped/failed posting details and direct relational `night_audit_id` linkage are absent. |
| Business Date Closing | Strong but has a governance gap | Audit claims and locks a hotel/date, blocks unsafe posting, closes only after posting/evaluation, and opens the next date. Multiple open dates are still possible because no database constraint enforces one open date per hotel. |
| Audit Summary | MVP-ready | Stored `run_results` reconcile run-specific status changes, posted charges, skipped items, and failed items; operational and financial summaries remain available. |
| Retry / Idempotency | Strong | Failed/blocked runs can retry; nightly/no-show charge keys and unique indexes prevent duplicate charges. Status changes completed before a later failure remain committed but are idempotently skipped on retry. |
...
|---|---|---|---|---|
| Missed arrival review | `confirmed` with passed arrival date becomes `review_no_show` | `Bookings::ReviewMissedArrivals` changes confirmed arrivals for the audited date to `review_no_show` and stores `no_show_review_business_date`. | Implemented | Called by `Bookings::ProcessNoShowReviews` from `HotelOps::RunNightAudit`. |
| Missed departure review | `checked_in` with passed departure date becomes `review_due_out` | `Bookings::ReviewDueOuts` transitions checked-in bookings due on or before the audited date; unresolved reviews are warnings while failed transitions leave stale checked-in blockers. | Implemented | Booking audit logs include `source: night_audit`, `night_audit_id`, and business date. |
...
| Extend due-out stay | `review_due_out` returns to `checked_in` after extending stay | `Bookings::ProcessLateCheckout` updates checkout date and transitions with `resolve_late_checkout`. | Implemented | Night Audit itself does not create `review_due_out`. |
...
| No direct checked-in to completed | Night Audit must not check guests out | Night Audit does not transition checked-in bookings to completed. | Implemented | Due-outs become blockers instead. |
...
| Skipped charge tracking | Store skipped charges and reason | Existing duplicate charges are silently returned/actualized; missing folios are skipped before post and later surfaced only as blockers. | Missing | No per-item skipped record or duplicate-skip reason in the audit summary. |
| Failed charge tracking | Store failed charges and error reason | A posting failure raises and fails the audit; only the overall error is stored in Night Audit logs/events. | Partially Implemented | No per-booking/per-charge failed item list. |
...
| One open date per hotel | Only one open business date per hotel | Unique index exists only on hotel/date, not on hotel where status is open. | Missing | Multiple different dates can simultaneously be `open`. |
...
| Summary snapshot | Store run summary/snapshot | JSON summary, blockers, exceptions, and separate financial summary are persisted. | Implemented | Run-level posting/skipped/failed detail is incomplete. |
...
| Separate status/posting results | Separate booking changes from folio postings | Operational summary and financial summary are separate, but run-specific change/posting lists are absent. | Partially Implemented |  |
...
## 6. Optional Enterprise Features Matrix
...
| Reopen closed date | Reopen a closed business date | Model has a `reopened` status and posting guard behavior, but no reviewed reopen service/controller transition was found. | Needs Review | Status exists without a clear operational workflow. |
...
|---|---|---|---|
| High | No database constraint guarantees only one open business date per hotel. | Multiple open dates can permit ambiguous posting/business-date governance. | Add a database-enforced single-current/open-date strategy after cleaning existing data and defining allowed blocked/reopened states. |
...
|---|---|---|---|---|---|
| High | Reporting | The displayed `review_no_show_count` is the total number currently in review, not the number moved by that audit run. | Historical or pre-existing review records can make the summary claim more status changes than the run performed. | `app/services/hotel_ops/evaluate_night_audit.rb#build_summary`; run-specific count exists only in process-start log metadata in `HotelOps::RunNightAudit`. | Store and display run-specific transition counters; label snapshot counts separately. |
| High | Business date governance | Multiple different `hotel_business_dates` rows can be `open` for one hotel. | Posting guard accepts any explicitly open date, creating ambiguous current business dates. | `db/schema.rb` has unique `(hotel_id, business_date)` only; `HotelBusinessDate` has no single-open validation/constraint. | Enforce one current open date per hotel at the database level or redesign around a single current-business-date pointer. |
| Medium | No-show workflow | A later Night Audit automatically finalizes every expired `review_no_show` and posts no-show charges. | The requested flow expects staff review actions; automatic charging/status finalization can create disputes when a booking is still awaiting resolution. | `Bookings::ProcessNoShowReviews#finalize_expired_reviews`; `Bookings::FinalizeNoShow`; test “keeps an expired review finalized when the audit later fails”. | Make auto-finalization an explicit hotel policy/enterprise option; default MVP to staff resolution. |
| Medium | Partial failure behavior | Missed-arrival/no-show status changes and charges remain committed when a later audit phase fails. | Retry is financially idempotent, but operators may see a failed audit that already changed bookings/posted no-show fees without a clear itemized partial-work summary. | `HotelOps::RunNightAudit` performs no-show processing before evaluation; specs confirm review/no-show changes persist after failure. | Record completed steps and affected items on the failed run; clearly display partial completion. |
| Medium | Reporting | Financial summary totals all transactions posted to the business date, not specifically transactions created by the Night Audit run. | The total can be correct for business-day revenue but cannot reconcile “charges posted by this audit,” and manual postings may be mistaken for audit postings. | `HotelOps::CalculateBusinessDayFinancials` filters only hotel and `posting_date`; no `night_audit_id` filter. | Present separate business-day totals and run-posted totals. |
...
| Low | Model/workflow consistency | `HotelBusinessDate::STATUSES` includes `reopened`, but no reviewed reopen transition workflow was found. | A state that can be posted to only with override exists without a clear controlled entry path. | `app/models/hotel_business_date.rb`; `app/services/financial_controls/posting_guard.rb`. | Remove the unused state or implement a permissioned, audited reopen workflow later. |
...
| Business date close | Yes | Implemented | Add single-open-date enforcement. |
...
| Failed audit retry safety | Yes | Implemented | Retain current idempotency and add explicit retry/partial-work visibility. |
...
1. Enforce one open business date per hotel and add targeted protection against booking timeline/status races during audit execution.
